...
│       │   ├── lanceService.ts          # LanceDB 嵌入式向量数据库存取
...
**架构**：LanceDB（嵌入式向量数据库）+ OpenAI Embeddings

- **Embedding 模型**: `text-embedding-3-small`（1536 维）
- **向量表**: `lightbulb_memories`（cosine 距离）
- **数据目录**: `data/lancedb/`（嵌入式，无需外部进程）
- **记忆类型**: `summary`（对话摘要）、`fact`（关键事实）
- **检索策略**: 用户消息 → embedding → LanceDB 向量检索 top 5 → 注入 system prompt
