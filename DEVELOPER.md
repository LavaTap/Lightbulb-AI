...
│       │   ├── planning/                # 素材面板组件
│       │   │   ├── PlanningCanvas.tsx   # 画布组件（@xyflow/react ReactFlow 配置 + 背景点/小地图/控制栏）
...
flowchart TD
    ...
    PP --> PC["PlanningCanvas<br/>ReactFlow 配置"]
...
| `components/planning/PlanningCanvas.tsx` | 画布组件，配置 ReactFlow（背景点/小地图/缩放控制/snapToGrid） |
...
**自动行为**：
- **URL 网页抓取**：发送消息时，后端自动检测用户消息中的 URL，调用 `webFetcher.enrichMessageWithUrls()` 抓取网页内容并注入 AI 上下文（静默降级，失败不影响对话）
