...
| **AI 对话** | 智能对话 + 长期记忆（ChromaDB 向量检索 + 自动摘要） | `ChatPage` |
...
│       │   ├── embeddingService.ts     # Embedding 生成服务（text-embedding-3-small）
│       │   ├── chromaService.ts        # ChromaDB 向量数据库连接与存取
│       │   └── memoryService.ts        # AI 记忆服务（摘要/事实提取/记忆检索注入）
│       └── routes/
...
│           ├── ChatPage.tsx           # AI 对话页（SSE 流式 + ChromaDB 记忆）
...
**架构**：ChromaDB（本地向量数据库）+ OpenAI Embeddings

- **Embedding 模型**: `text-embedding-3-small`（1536 维）
- **向量集合**: `lightbulb_memories`
- **记忆类型**: `summary`（对话摘要）、`fact`（关键事实）
- **检索策略**: 用户消息 → embedding → 查询 ChromaDB top 5 → 注入 system prompt
- **摘要触发**: 消息数 > 20 且距上次摘要新增 > 10 条时自动触发
- **降级机制**: ChromaDB 不可用时对话正常工作，仅记忆功能关闭
...
# 5. （可选）启动 ChromaDB 以启用 AI 对话记忆功能
pip install chromadb && chroma run --path ./data/chromadb --port 8000
```
...
> **Changelog (v2.5 | 2026-05-03)**:
> - 新增 AI 对话功能模块（ChatPage + chat 路由）
> - 新增 ChromaDB 向量数据库集成，支持 AI 长期记忆
