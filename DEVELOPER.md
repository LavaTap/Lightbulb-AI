...
interface GenerationRecord {
  ...
  prompt?: string;
  uploadImages: string;       // base64, 逗号分隔多图
  generatedImages: string;    // base64
...
| `upload_images` | TEXT | - | 上传图片 base64（逗号分隔多图） |
| `generated_images` | TEXT | - | 生成图片 base64（逗号分隔多图） |
...
- **图片体积**：所有图片以 base64 传输，注意前端压缩（已实现 200px 缩略图存 DB）和后端 50MB body limit
