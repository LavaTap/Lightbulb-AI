...
- [4 后端架构详解](#4-后端架构详解)
- [5 前端架构详解](#5-前端架构详解)
  - [5.9 素材面板（规划页面）](#59-素材面板规划页面)
  - [5.10 知识路由机制](#510-知识路由机制)
- [6 模型连接机制](#6-模型连接机制)
...
```mermaid
graph TB
    subgraph Browser["用户浏览器 (React SPA :3000)"]
        Pages["灵感提示 / AI对话 / 文生图 / 海报生成 / 素材面板"]
    end
    Browser -->|"/api" Vite Proxy| Server["Express Server (:3001)"]
    
    subgraph Server["Express Server (:3001)"]
        Routes["/api/generate<br/>/api/records<br/>/api/model-configs"]
        Services["Service Layer<br/>visionService / imageGenService / posterService<br/>chatService / memoryService / lanceService"]
        DB[("SQLite<br/>data/lightbulb.db")]
        Routes --> Services --> DB
    end
    
    Services --> AI["AI Providers<br/>OpenAI / Aliyun / Google / ..."]
```
...
| **流程图引擎** | @xyflow/react (React Flow) | latest |
| **HTTP 客户端** | Axios | ^1.7.7 |
| **状态管理** | React Hooks (useState/useContext) | - |
| **后端框架** | Express.js + TypeScript | ^4.21.0, ^5.6.2 |
| **数据库** | sql.js (嵌入式 SQLite) | ^1.11.0 |
| **向量数据库** | @lancedb/lancedb (LanceDB) | ^0.27.2 |
| **AI SDK** | OpenAI Node.js SDK | ^4.68.2 |
| **请求校验** | Zod | ^3.23.8 |
| **文件解析** | mammoth (docx) + pdf-parse (pdf) + xlsx (xlsx) | latest |
| **网页抓取** | cheerio (HTML 解析) | latest |
...
│       │   ├── fileParser.ts           # 文件解析服务（docx→mammoth / pdf→pdf-parse / xlsx→xlsx / 文本→UTF-8）
│       │   ├── webFetcher.ts           # 网页抓取服务（消息含 URL 时自动抓取 → cheerio 提取可读文本）
...
│       │   └── planning.ts             # 素材面板类型定义（TextNodeData/ImageNodeData/PlanningNodeType）
│       │
│       ├── lib/
│       │   └── utils.ts               # 工具函数（cn/imageToBase64/compressImage/formatTokens/fileToBase64/formatFileSize/getFileIcon...）
│       │
│       ├── services/
...
│       │   └── usePlanningState.ts     # 素材面板状态管理 Hook（节点/边/视口/localStorage 持久化）
│       │
│       ├── components/
│       │   ├── Layout.tsx               # 布局壳（Header + main + footer + 记录弹窗）
│       │   ├── Header.tsx               # 导航栏（Tab 切换 + Logo + 鼠标跟随效果）
│       │   ├── AvatarMenu.tsx           # 头像菜单（模型管理/记录/用量统计/主题切换）
...
│       │   ├── ImagePreview.tsx         # 图片放大预览弹窗（缩放/旋转/拖拽/下载/导航）
│       │   ├── ImageUploadZone.tsx      # 图片上传区域（拖拽+点击+压缩）
│       │   ├── RecordDetailDialog.tsx    # 记录详情弹窗（图片/提示词/元信息）
│       │   ├── ThemeToggle.tsx          # 亮暗模式切换按钮
│       │   ├── UsageStatistics.tsx      # 用量统计面板（模型分布饼图/Token折线图/模型Token柱状图，支持周/月/年/全部切换）
│       │   ├── planning/                # 素材面板组件
│       │   │   ├── PlanningCanvas.tsx   # 画布组件（@xyflow/react ReactFlow 配置 + 背景点/小地图/控制栏 + 拖放导入图片）
│       │   │   ├── PlanningToolbar.tsx  # 工具栏（添加文本/图片/删除/缩放/适应视图）
│       │   │   └── nodes/
│       │   │       ├── TextNode.tsx     # 文本节点（双击编辑，4 方向 Handle 连接点）
│       │   │       └── ImageNode.tsx    # 图片节点（拖拽缩放，4 方向 Handle 连接点）
│       │   └── ui/                     # shadcn/ui 组件
│       │       ├── button.tsx
│       │       ├── card.tsx
│       │       ├── dialog.tsx
...
│           ├── CharacterGenPage.tsx     # 文生图页
│           ├── ThreeViewPage.tsx       # 角色三视图页
│           ├── PosterGenPage.tsx       # 海报生成页
│           ├── CgGenPage.tsx           # CG 生成页（占位）
│           ├── PlanningPage.tsx        # 素材面板页（@xyflow/react 可视化画布 + ReactFlowProvider）
...
| 对话标题生成 | 首次对话后异步触发 | 调用 AI 模型自动生成对话标题 |
...
| POST | `/:id/test-connection` | **测试 API 连通性** |
...
| 方法 | 路径 | 功能 |
|------|------|------|
| GET | `/statistics` | 用量统计（period 参数: week/month/year/all，排除 chat 类型） |
...
graph LR
    ...
    B --> C["&lt;App /&gt;"]
    C --> D["useState&lt;FeatureType&gt;('inspiration')"]
    D --> E["switch(activeTab) 渲染对应 Page"]
    D --> F["Layout (Header + main + footer)"]
```
...
| `recordsApi` | `.getAll(page, pageSize)` / `.create(data)` / `.delete(id)` / `.getStatistics(period)` |
...
graph TB
    ...
    Layout --> Header["&lt;Header&gt; (Tabs + Logo + AvatarMenu)"]
    Layout --> Main["&lt;main&gt;"]
    Main --> Animate["&lt;AnimatePresence&gt;"]
    Animate --> Page["&lt;Page/&gt; (按 activeTab 切换)"]
    Layout --> Footer["&lt;footer&gt;"]
    Layout --> Dialog["&lt;Dialog&gt; (RecordsPage embedded)"]
```
...
| `UsageStatistics` | 用量统计面板（从 AvatarMenu 打开，recharts 图表：模型分布饼图/Token折线图/模型Token柱状图，周/月/年/全部切换，暗色模式适配） |
...
#### 对话标题自动生成
- 首次对话后，后端异步调用 AI 模型自动生成对话标题（`generateConversationTitle()`）
- 前端在流式结束后延迟 1.5 秒刷新对话标题
- 新标题自动更新侧边栏对话列表
...
#### 附件上传与展示
- **统一上传按钮**：图片和文件上传合并为一个 `Plus` 按钮，点击弹出 `DropdownMenu` 选择"图片"或"文件"
- **附件预览条**：输入框上方展示已添加的附件列表
  - 图片附件：`w-20 h-20` 缩略图卡片，底部渐变遮罩显示文件名
  - 文件附件：`w-44` 横向卡片，显示文件图标、文件名和文件大小
- **消息气泡中的附件展示**：
  - 图片：`w-24 h-24` 正方形缩略图，点击可放大预览
  - 文件：横向卡片样式，显示文件图标 + 文件名 + 文件大小
- **文件读取**：后端将文本文件（通过 base64 解码）内容注入到消息中，格式为 `--- 附件: filename ---\ncontent\n--- 附件结束 ---`

#### URL 网页内容自动抓取
- 发送消息时，后端自动检测用户消息中的 URL 链接
- 调用 `webFetcher.enrichMessageWithUrls()` 抓取网页内容
- 抓取结果注入到 AI 上下文，格式为 `--- 网页内容: {url} ---\n{content}\n--- 网页内容结束 ---`
- 使用 cheerio 提取可读文本，移除 script/style/nav/footer 等噪音
- 支持 OG/Twitter 标题优先提取，8000 字符截断
- 抓取失败不影响对话正常进行（静默降级）

#### 多模态错误处理
- **模型不支持图片**：后端检测 400 错误（含 `image_url`/`multimodal` 关键词），发送 `event: warning` SSE 事件，剥离附件后重试纯文本请求；前端移除用户消息中的附件并显示警告
- **模型类型不匹配**：后端检测 404 错误（含 `page not found`），发送 `event: error` 提示用户切换到正确的文本模型

### 5.9 素材面板（规划页面）

素材面板基于 **@xyflow/react**（React Flow）实现，是一个可视化画布编辑器，支持文本节点、图片节点和连线。

#### 架构概览

```mermaid
flowchart TD
    PP["PlanningPage<br/>(ReactFlowProvider)"] --> PT["PlanningToolbar<br/>添加文本/图片/删除/缩放"]
    PP --> PC["PlanningCanvas<br/>ReactFlow 配置 + 拖放导入"]
    PC --> TN["TextNode<br/>双击编辑 + 4方向Handle"]
    PC --> IN["ImageNode<br/>拖拽缩放 + 4方向Handle"]
    PP --> UPS["usePlanningState<br/>状态管理 + localStorage持久化"]
```
...
| 文件 | 职责 |
|------|------|
| `pages/PlanningPage.tsx` | 页面入口，包裹 `ReactFlowProvider`，组合 Toolbar + Canvas |
| `hooks/usePlanningState.ts` | 状态管理 Hook（节点/边/视口 CRUD + localStorage 持久化） |
| `components/planning/PlanningCanvas.tsx` | 画布组件，配置 ReactFlow（背景点/小地图/缩放控制/snapToGrid），新增拖放导入图片功能 |
| `components/planning/PlanningToolbar.tsx` | 工具栏（添加文本/图片/删除/缩放/适应视图） |
| `components/planning/nodes/TextNode.tsx` | 文本节点（双击编辑，4 方向 Handle） |
| `components/planning/nodes/ImageNode.tsx` | 图片节点（拖拽缩放，4 方向 Handle） |
| `types/planning.ts` | 类型定义（TextNodeData / ImageNodeData / PlanningNodeType） |

#### 连线功能

- **连接点（Handle）**：每个节点有 4 个 Handle（Top/Bottom/Left/Right），各含 `target` + `source` 类型
- **交互**：从一个 Handle 拖拽到另一个 Handle 即创建连线
- **Handle 显示**：hover 时才显示（`opacity-0 group-hover:opacity-100`）
- **边样式**：`smoothstep` 类型（折线），紫色 `#8b5cf6`，2px 宽度
- **删除**：选中边后按 Delete/Backspace 键

#### 插图（图片节点）

- **上传入口**：工具栏 `ImagePlus` 按钮 → 隐藏 `<input type="file" accept="image/*">`
- **处理流程**：`FileReader.readAsDataURL()` → DataURL → `addImageNode(dataUrl)`
- **ImageNode 特性**：
  - 默认尺寸 240×180，保持宽高比缩放
  - 右下角拖拽缩放手柄（自定义 mousedown/mousemove/mouseup 实现）
  - 最小宽度 100px，最小高度 60px
  - 图片以 DataURL 存储在 `node.data.imageSrc`

#### 拖放导入图片

- **入口**：整个画布区域支持拖放，从文件管理器拖入图片文件即可
- **支持格式**：JPG、JPEG、PNG、GIF、WebP、BMP、ICO、SVG 所有常见图片格式
- **位置定位**：图片创建在鼠标松开位置，自动通过 `screenToFlowPosition` 转换为画布坐标
- **多图片处理**：同时拖入多个图片时，自动错开位置摆放（每张偏移 40px），避免完全重叠
- **格式验证**：双重验证（MIME 类型 + 文件扩展名），确保兼容性
- **视觉反馈**：拖拽进入画布时显示半透明蓝色覆盖层，提示"松开鼠标导入图片"
- **暗黑模式适配**：提示文字颜色自动适配深色主题

#### 插入文字（文本节点）

- **添加方式**：工具栏"添加文本"按钮 → `addTextNode()` → 创建 `planningText` 节点
- **初始文字**：`'双击编辑'`
- **编辑交互**：
  - 双击节点 → 进入编辑模式（`<textarea>` 替换 `<div>`）
  - `textarea` 添加 `nodrag nowheel` class 防止拖拽冲突
  - Enter（非 Shift）→ 保存退出；Escape → 取消编辑；失焦 → 自动保存

#### 状态持久化

| 属性 | 值 |
|------|-----|
| 存储 key | `lightbulb_planning_canvas` |
| 存储内容 | `{ nodes, edges, viewport }` JSON |
| 防抖间隔 | 500ms |
| 触发时机 | nodes/edges 变化时自动保存 |
| 恢复时机 | 组件 mount 时从 localStorage 读取，恢复节点、边和视口位置 |

### 5.10 知识路由机制

项目使用双文档知识体系（`CLAUDE.md` + `DEVELOPER.md`），根据问题类型自动路由到最合适的知识源：

```mermaid
flowchart TD
    Start["用户提问"] --> Type["问题类型分析"]

    Type -->|"架构/数据流问题"| ClaudeRoute["路由到 CLAUDE.md"]
    Type -->|"功能实现问题"| DeveloperRoute["路由到 DEVELOPER.md"]
    Type -->|"未明确匹配"| DefaultRoute["默认 DEVELOPER.md"]

    ClaudeRoute --> ClaudeProcess["处理：架构概述 + 技术栈 + 图表"]
    DeveloperRoute --> DevProcess["处理：详细实现 + API文档 + 代码示例"]
    DefaultRoute --> DevProcess

    ClaudeProcess --> Output["输出回答"]
    DevProcess --> Output
```

| 问题类型 | 路由目标 | 回答侧重 |
|---------|---------|---------|
| 项目结构、整体架构、技术栈概览 | `CLAUDE.md` | 架构概述 + 技术选型 + 架构图 |
| 模块实现、API 接口、数据库字段、代码细节 | `DEVELOPER.md` | 详细实现 + API 文档 + 代码示例 |
| 新增功能/服务商扩展流程 | `CLAUDE.md` + `DEVELOPER.md` | CLAUDE.md 的模式指南 + DEVELOPER.md 的详细步骤 |
| 未明确匹配 | `DEVELOPER.md` | 默认查手册，未覆盖则读源码补充 |
...
```mermaid
flowchart TD
    A["用户点击'生成图片'"] --> B["CharacterGenPage"]
    B --> C["useGeneration().generate(prompt)"]
    
    subgraph Hook["[Hook 内部]"]
        D["useApiConfig → getCurrentProvider() + getConfig(provider)"]
        E["imageApi.generate({ prompt, config, size })"]
        D --> E
    end
    ...
    C --> Hook["[Hook 内部]"]
    Hook --> F["POST /api/image/generate"]
    
    subgraph Backend["[后端 routes/generate.ts]"]
        G["generateImageSchema 校验"]
        H["imageGenService.generate(prompt, config, size)"]
        G --> H
    end
    
    F --> Backend
    
    subgraph Service["[imageGenService]"]
        I{"config.model 匹配?"}
        I -->|"startsWith 'qwen-image'"| J["阿里云 DashScope API"]
        I -->|"startsWith 'hy-image'"| K["腾讯混元 API<br/>(TC3签名/TokenHub)"]
        I -->|"=== 'gpt-image-2'"| L["gptimage2 代理 API<br/>(异步轮询)"]
        I -->|"其他"| M["OpenAI Images API"]
    end
    
    H --> Service
    
    Service --> N["返回 { imageBase64, tokenUsage }"]
    
    subgraph Hook2["[Hook 后续]"]
        O["compressImage(200px 缩略图)"]
        P["recordsApi.create(record)"]
        Q["返回 base64 给页面"]
        O --> P --> Q
    end
...
| `total_tokens_used` | INTEGER | DEFAULT 0 | 总 Token 消耗量 |
| `period_start` | DATETIME | NOT NULL | 统计周期开始时间 |
| `period_end` | DATETIME | NOT NULL | 统计周期结束 |
| `created_at` | DATETIME | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
...
**GET** `/api/records/statistics?period=week` → 用量统计

- `period` 参数: `week`(默认, 近7天) / `month`(近30天) / `year`(近365天) / `all`(全部)
- 排除 `feature_type = 'chat'` 的记录
- 响应:
```json
{
  "totalRecords": 42,
  "totalTokens": 123456,
  "modelDistribution": [{ "modelName": "gpt-4o", "count": 20 }],
  "dailyTokenUsage": [{ "date": "2026-05-01", "totalTokens": 5000 }],
  "tokenUsageByModel": [{ "modelName": "gpt-4o", "totalTokens": 80000 }]
}
```
...
请求：`{ "content": "你好", "config": { ...APIConfig }, "attachments?": [{ "type": "image|file", "dataBase64": "...", "mimeType": "...", "fileName": "...", "fileSize?": 1234 }] }`
...
event: warning        → { "warning": "...", "type": "multimodal_not_supported" }
...
**自动行为**：
- **URL 网页抓取**：发送消息时，后端自动检测用户消息中的 URL，调用 `webFetcher.enrichMessageWithUrls()` 抓取网页内容注入 AI 上下文（静默降级，失败不影响对话）
- **对话标题自动生成**：首次对话后，后端异步调用 `generateConversationTitle()` 自动生成对话标题，前端延迟 1.5 秒刷新
...
```mermaid
flowchart TD
    A["用户上传图片"] --> B["选择分析类型<br/>(character/landscape/object/other)"]
    B --> C["[前端] InspirationPage"]
    C --> D["POST /api/vision/analyze<br/>(imageBase64 + config + category)"]
    
    subgraph Backend["[后端] visionService.analyzeImage()"]
        E["根据 category 生成差异化 prompt"]
        F["调用 qwen-vl-plus (DashScope)"]
        G["解析 JSON 响应"]
        E --> F --> G
    end
    
    D --> Backend
    Backend --> H["返回 { analysis: { zh, en } }"]
    
    subgraph Frontend["[前端] 显示分析结果"]
        I["提供 中/英/JSON 三种格式查看"]
        J["一键复制"]
        K["自动保存记录到 generation_records"]
    end
...
```mermaid
flowchart TD
    A["用户输入提示词"] --> B["选择 text-to-image 类型的模型"]
    B --> C["[前端] CharacterGenPage"]
    C --> D["POST /api/image/generate<br/>(prompt + config + size)"]
    
    subgraph Backend["[后端] imageGenService.generateImage()"]
        E{"config.model 前缀判断"}
        E -->|"qwen-image"| F["阿里云 DashScope API<br/>(支持参考图图生图)"]
        E -->|"hy-image"| G["腾讯混元 API<br/>(TC3-HMAC-SHA256 或 TokenHub)"]
        E -->|"其他"| H["OpenAI Images API (DALL-E 等)"]
    end
    
    D --> Backend
    Backend --> I["返回 { imageBase64 }"]
    
    subgraph Frontend["[前端] 显示生成的角色图片"]
        J["下载按钮（PNG）"]
        K["自动保存记录"]
    end
...
```mermaid
flowchart TD
    A["上传参考图 + 输入提示词（必填）"] --> B["选择 image-to-image 类型模型"]
    B --> C["[前端] ThreeViewPage"]
    
    subgraph Loop["[循环 3 次] POST /api/image/generate (size=1792x1024)"]
        D["第 1 次: '{prompt}, front view,<br/>character design sheet'"]
        E["第 2 次: '{prompt}, side view,<br/>character design sheet'"]
        F["第 3 次: '{prompt}, back view,<br/>character design sheet'"]
    end
    
    C --> Loop
    Loop --> G["返回 3 张 base64 图片"]
    
    subgraph Frontend["[前端] 以 16:9 比例显示"]
        H["正面 / 侧面 / 背面 标签"]
        I["下载单张或全部"]
        J["自动保存记录<br/>(generatedImages 逗号拼接 3 张图)"]
    end
...
```mermaid
flowchart TD
    A["上传角色参考图（必填，最多 5 张）"] --> B["输入描述（必填）"]
    B --> C["选择海报尺寸<br/>(竖版9:16 / 横版16:9 / 正方形1:1)"]
    C --> D["选择 image-to-image 类型模型"]
    D --> E["POST /api/poster/generate<br/>(images[] + prompt + config + size)"]
    
    subgraph Backend["[后端] posterService.generatePoster()"]
        F["Step 1: analyzeImage(第1张参考图)<br/>→ 提取画风特征"]
        G["Step 2: 组合 prompt<br/>'Poster design: {userPrompt}. Style: {analysis.en}'"]
        H["Step 3: generateImage(enhancedPrompt, config, size)"]
        F --> G --> H
    end
    
    E --> Backend
    Backend --> I["返回海报 base64"]
    
    subgraph Frontend["[前端] 按所选尺寸比例展示"]
        J["下载"]
        K["自动保存记录"]
    end
...
- **适用场景**：角色三视图（图生图）、文生图（文生图）、海报生成

---

> **Changelog (v3.2 | 2026-05-05)**:
> - 新增：对话标题自动生成——首次对话后后端异步调用 AI 模型生成对话标题（`generateConversationTitle()`），前端延迟 1.5 秒刷新
> - 新增：URL 网页内容自动抓取——发送消息时后端自动检测 URL，调用 `webFetcher.enrichMessageWithUrls()` 抓取网页内容注入 AI 上下文（静默降级）
> - 新增：`useChat` Hook 支持 `overrideConfig` 参数，可传入自定义模型配置覆盖默认选择
> - 新增：`formatRelativeTime` / `base64ToDataUrl` / `fileToBase64` / `formatFileSize` / `getFileIcon` 工具函数
> - 新增：`MessageAttachment` / `ChatMessage` / `Conversation` / `ProviderInfo` 前端类型定义
> - 优化：ChatPage 附件上传 UI（统一 Plus 下拉菜单、图片缩略图卡片、文件卡片）
> - 优化：ChatPage 侧边栏折叠动画、滚动到底部按钮
> - 优化：消息气泡中文件附件以横向卡片展示（File 图标 + 文件名 + 大小）
>
> **Changelog (v3.4 | 2026-05-08)**:
> - 新增：素材面板（规划页面）——基于 @xyflow/react (React Flow) 的可视化画布编辑器
>   - `PlanningPage.tsx`：页面入口，包裹 ReactFlowProvider
>   - `usePlanningState.ts`：状态管理 Hook（节点/边/视口 CRUD + localStorage 防抖持久化）
>   - `PlanningCanvas.tsx`：画布组件（背景点/小地图/缩放控制/snapToGrid）
>   - `PlanningToolbar.tsx`：工具栏（添加文本/图片/删除/缩放/适应视图）
>   - `TextNode.tsx`：文本节点（双击编辑，Enter 保存，Escape 取消，4 方向 Handle）
>   - `ImageNode.tsx`：图片节点（文件上传 → DataURL，拖拽缩放，4 方向 Handle）
>   - `planning.ts`：类型定义（TextNodeData / ImageNodeData / PlanningNodeType）
> - 新增：§5.9 素材面板（规划页面）完整文档（架构图/核心文件/连线/插图/插入文字/状态持久化）
> - 新增：§5.10 知识路由机制（CLAUDE.md vs DEVELOPER.md 路由策略 Mermaid 流程图）
> - 更新：§1 项目概述新增素材面板功能行
> - 更新：§2 技术栈新增 @xyflow/react 依赖 + 架构图新增素材面板
> - 更新：§3 目录结构新增 planning/ 组件目录、usePlanningState.ts、planning.ts、PlanningPage.tsx
>
> **Changelog (v3.3 | 2026-05-06)**:
> - 新增：用量统计功能——`GET /api/records/statistics?period=week|month|year|all`，支持按时间段查询统计（模型分布/每日Token/模型Token用量）
> - 新增：`UsageStatistics.tsx` 组件——recharts 图表（模型分布饼图、Token折线图、模型Token柱状图），周/月/年/全部切换，暗色模式适配
> - 变更：用量统计入口从 RecordsPage 移至 AvatarMenu 头像下拉菜单，独立 Dialog 展示
> - 变更：统计维度从服务商分布改为模型分布（`model_name` 而非 `model_provider`）
> - 新增：前端 recharts 依赖
>
> **Changelog (v3.1 | 2026-05-05)**:
> - 新增：网页抓取服务 `webFetcher.ts`，用户发送链接时自动抓取网页内容注入 AI 上下文（使用 cheerio 提取可读文本）
> - 修复：`fileParser.ts` 中 PDF 解析（pdf-parse v2）使用错误 API——改为 `new PDFParse({ data: buffer }).getText()`
> - 新增：文件解析服务 `fileParser.ts`，支持 `.docx`（mammoth）、`.pdf`（pdf-parse）、`.xlsx/.xls`（xlsx）、纯文本文件
> - 优化：历史消息中文件附件由简单 UTF-8 解码改为智能路由解析，二进制文档（Word/PDF/Excel）自动提取文本内容
>
> **Changelog (v3.1 | 2026-05-05)**:
> - 新增：文件附件上传功能（`fileToBase64` 工具函数 + `MessageAttachment.type` 扩展 `'file'` + `fileSize` 字段）
> - 新增：后端文件内容注入——文本文件通过 base64 解码后拼接到消息中（`--- 附件: filename ---\ncontent\n--- 附件结束 ---`）
> - 新增：多模态错误处理——后端检测 400/404 错误并发送 `event: warning` SSE 事件，前端移除附件并显示警告
> - 新增：模型类型不匹配检测——纯图像生成模型被用于对话时提示用户切换
> - 新增：`DropdownMenu` shadcn/ui 组件封装
> - 优化：附件上传按钮合并为统一 `Plus` 下拉菜单（图片/文件）
> - 优化：附件预览卡片 UI——图片卡片 `w-20 h-20` 带渐变文件名，文件卡片 `w-44` 显示图标+文件名+大小
> - 优化：消息气泡中文件附件展示——从错误图片渲染改为横向文件卡片
> - 新增：`formatFileSize` / `getFileIcon` 工具函数
>
> **Changelog (v3.0 | 2026-05-04)**:
> - 重构：§2 架构图、§5.1 入口与应用、§5.6 组件树、§6.4 调用流程、§9 全部核心业务流程从纯文本 ASCII 图升级为 Mermaid 图表
> - 更新：GUIDE skill 文档新增 Mermaid 图表输出要求，回答架构/数据流类问题时自动附带 Mermaid 图表
>
...
> - 前端 `APIConfig.provider` 类型新增 `aliyun` / `bytedance` / `baidu`（原仅 5 家）
> - 前端 `ModelCategory` 类型新增 `text`（纯文本模型）
> - 重构：海报生成新增 7 种尺寸选项（竖版/横版/正方形各 1080p 和 2K）
> - 重构：三视图输出锁定 16:9 (2560x1440) 2K 横向宽屏
> - 数据库 schema 无变更（向下兼容）
>
> **Changelog (v3.5 | 2026-05-09)**:
> - 新增：素材面板拖放导入图片功能——整个画布区域支持拖入图片文件
>   - 支持所有常见图片格式：JPG、JPEG、PNG、GIF、WebP、BMP、ICO、SVG
>   - 图片自动创建在鼠标松开位置，通过 `screenToFlowPosition` 正确转换为画布坐标
>   - 同时拖入多个图片时自动错开位置摆放（每张偏移 40px）
>   - 双重格式验证：MIME 类型 + 文件扩展名，确保兼容性
>   - 拖拽进入时显示半透明蓝色覆盖层视觉反馈，提示"松开鼠标导入图片"
>   - 适配暗黑模式，提示文字颜色自动匹配主题
> - 更新：`components/planning/PlanningCanvas.tsx` 增加 `onAddImageNode` props 和拖拽事件处理
> - 更新：`pages/PlanningPage.tsx` 传递 `onAddImageNode` 回调给 `PlanningCanvas`
> - 更新：§5.9 素材面板新增"拖放导入图片"小节文档
