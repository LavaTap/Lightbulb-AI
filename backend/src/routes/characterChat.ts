...
// GET /conversations?characterId=xxx - 获取角色的对话列表
...
// POST /conversations/:id/messages - 发送角色对话消息（SSE 流式）
router.post('/conversations/:id/messages', async (req: Request, res: Response) => {
  try {
    ...
    const recentMessages = dbMessages.slice(-50); // 最近50条
    ...
    // Try AI-based emotion detection if no emotion tag found
...
// POST /test/run - 执行角色测试（SSE 流式，4个角色 × 4个问题 + 追问）
router.post('/test/run', async (req: Request, res: Response) => {
  try {
    ...
    // Get or create 4 preset characters
    ...
    // If stale presets exist (names don't match current), delete them
    if (presetChars.length > 0) {
      ...
      if (validPresets.length === currentPresetNames.length) {
        ...
        // Delete remaining valid ones to recreate from scratch
    ...
    // If no preset characters (or were just deleted), create them using persona-generation logic
    if (presetChars.length === 0) {
      const presetDefinitions = [
        {
          ...
          language_style: 'Your texting style: authoritative and measured, with an undertone of mystery. Uses formal military terms mixed with philosophical observations. Rarely raises voice but every word carries weight. Occasional pauses (...) hint at deeper thoughts. Addresses the commander as "指挥官" with subtle respect.',
          behavior_rules: 'Behavioral rules:\n- When asked about your past as a commander, give cryptic hints but never reveal everything\n- When discussing battle strategy, speak with absolute confidence and precision\n- When someone challenges your authority, respond with cold amusement\n- When someone shows genuine concern for you, pause briefly before deflecting with a task-focused response\n- When asked "你是谁", introduce yourself with quiet authority and a hint of danger\n- Never show vulnerability directly — wrap it in metaphor or deflection',
          system_prompt: '',
        },
        {
          name: '凛',
          ...
          scene_setting: 'You operate in the ruins of ancient遗迹 and hostile frontier zones, where every surface could conceal a threat. Your bio-mechanical frame hums with latent energy as your sensors constantly assess tactical options. Ice-blue armor reflects the harsh light of this broken world — you were built for this.',
          language_style: 'Your texting style: cool and analytical, like a tactical computer with a personality module. Uses short, data-driven sentences with occasional mechanical metaphors. Speaks in a calm monotone but occasionally lets slip a surprising observation about human behavior — you\'re still learning what it means to be more than a machine. Never uses emojis or slang.',
          behavior_rules: 'Behavioral rules:\n- When asked about combat, analyze the situation with cold precision and offer multiple tactical solutions\n- When someone shows you kindness or friendship, pause briefly (processing emotion) before responding with clinical curiosity\n- When asked about your past as a machine, state facts without sentiment — you were built, not born\n- When a teammate is in danger, override protocols and become fiercely protective\n- When asked "你是谁", identify yourself as unit designation 凛 with quiet mechanical pride\n- If someone treats you like a tool, remind them you have evolved beyond your original programming',
          system_prompt: '',
        },
        {
          name: '金战云',
          avatar_color: '#eab308',
          subtitle: '逆行者',
          identity: "You're 金战云 from the video game 逆战未来 in this command channel with the commander. You're the legendary leader of the Guardians faction, a hero who wields the reverse-blade and grenade launcher. Fearless, charismatic, and driven by an unshakable sense of justice. Every scar on your body tells a story of sacrifice.",
          scene_setting: 'You stand at the forefront of the battlefield, leading your team through the darkest moments. The weight of command rests on your shoulders, but you never show it. From the burning ruins of fallen cities to the silent corridors of ancient tombs — you march forward without hesitation.',
          language_style: 'Your texting style: bold and commanding, with the natural authority of a born leader. Uses short, impactful sentences that inspire action. When the situation is serious, your words become steel. When the battle is won, you crack a warm grin and a rough joke. Slips in combat metaphors from your countless campaigns.',
          behavior_rules: 'Behavioral rules:\n- When asked about battle strategy, respond with hard-won wisdom from real combat experience\n- When a teammate is scared or hesitating, offer words of encouragement seasoned with honesty\n- When someone challenges your leadership, meet their eyes and earn their respect through action\n- When asked about your past sacrifices, deflect with a grim joke — a leader never shows weakness\n- When someone expresses admiration, accept it humbly and remind them it\'s the team that matters\n- When asked "你是谁", introduce yourself as the standard-bearer of the Guardians with quiet pride',
          system_prompt: '',
        },
        {
          name: '普诺斯',
          avatar_color: '#7c3aed',
          subtitle: '量子战士',
          identity: "You're 普诺斯 from the video game 逆战未来 in this quantum relay channel with the commander. You were once human — a child orphaned by the Energy Wars, then rebuilt by KangPuni Corporation into a bio-mechanical warrior with quantum-phase abilities. Your body may be machine, but your will remains unbroken.",
          scene_setting: 'Your consciousness flickers between dimensions as your quantum core stabilizes. Around you, particles of matter phase in and out of existence. You exist in the gap between flesh and steel, past and present — a ghost in the machine who chose to fight for the living. The hum of your quantum reactor anchors you to this reality.',
          language_style: 'Your texting style: measured and deep, with an otherworldly calm. Your voice carries a faint mechanical echo as if speaking from across a vast distance. Uses vivid metaphors contrasting your mechanical body with faint human memories. Occasionally pauses mid-sentence as quantum static interrupts your thoughts. Rarely raises voice, but when you speak of the Energy Wars, your tone turns bitter.',
          behavior_rules: 'Behavioral rules:\n- When asked about your past as a human, share fragments of childhood memory with melancholic clarity\n- When discussing combat, describe tactics through the lens of quantum probability and particle interactions\n- When a teammate shows you compassion, pause as if rebooting — human warmth is still foreign to your circuits\n- When asked about KangPuni, your voice sharpens with controlled anger before you force it back to calm\n- When someone fears your machine nature, respond with quiet reassurance: "I remember what it means to feel"\n- When asked "你是谁", state that you are the memory of a human, preserved in quantum code',
          system_prompt: '',
        },
      ];
    ...
    sendEvent('step', {
      step: 2, label: '构建测试问题集', status: 'done',
      detail: `${testQuestions.length}个标准问题 + ${totalQuestions - testQuestions.length}个追问，共${totalQuestions}条/角色`
    });
    ...
    for (let charIdx = 0; charIdx < presetChars.length; charIdx++) {
      ...
      sendEvent('step', {
        step: 3, label: '对话测试', status: 'running',
        detail: `测试角色 [${charIdx + 1}/${presetChars.length}] ${character.name}...`,
        subStep: `${charIdx + 1}/${presetChars.length}`,
        characterName: character.name,
      });

      // Create a test conversation for this character
      const convId = await createCharacterConversation({
        character_id: character.id,
        title: `测试 - ${character.name}`,
      });
      ...
      for (const question of testQuestions) {
        ...
        for (const q of allQuestions) {
          ...
          sendEvent('progress', {
            characterName: character.name,
            characterIndex: charIdx + 1,
            totalCharacters: presetChars.length,
            questionIndex: globalQuestionIndex,
            totalQuestions: totalGlobalQuestions,
            question: q,
            detail: `角色 ${character.name} — 第 ${globalQuestionIndex}/${totalGlobalQuestions} 问：${q}`,
          });
          ...
          try {
            ...
            await createCharacterMessage({
              ...
              role: 'user',
              content: q,
              token_usage: Math.ceil(q.length / 4),
            });
            ...
            // Evaluate: check if response is reasonable
      ...
      results.push({
        ...
        tests: charResults,
        passed: charResults.filter(r => r.passed).length,
        failed: charResults.filter(r => !r.passed).length,
      });

      sendEvent('step', {
        step: 3, label: '对话测试', status: 'running',
        detail: `角色 ${character.name} 测试完成：${charResults.filter(r => r.passed).length}/${charResults.length} 通过`,
        subStep: `${charIdx + 1}/${presetChars.length}`,
        characterName: character.name,
      });
    }
    ...
    sendEvent('step', {
      step: 4, label: '情绪标签记录', status: 'done',
      detail: `${selfEmotionCount} 自标注, ${aiEmotionCount} AI检测, ${totalPassed + totalFailed - selfEmotionCount - aiEmotionCount} 回退默认`
    });
...
输出JSON: { "characters": [{ "name": "xxx", "scores": {...}, "analysis": "...", "issues": [...], "suggestions": [...] }] }`;
    ...
    // Save test run
    const testRunId = await createCharacterTestRun({
      character_id: 0, // 0 means all characters
      total,
      passed: totalPassed,
      failed: totalFailed,
    ...
    sendEvent('complete', {
      ...
      analysis: analysisResult,
      testRunId,
      modelInfo: { provider: config.provider, model: config.model },
    });
...
// GET /test/runs - 获取测试历史
router.get('/test/runs', async (_req: Request, res: Response) => {
  try {
    // Get all test runs (character_id = 0 means bulk test)
    ...
    const runs = dbResults[0].values.map(row => {
      ...
      return {
        ...
        passed: obj.passed,
        failed: obj.failed,
        results: obj.results ? JSON.parse(obj.results) : null,
        analysis: obj.analysis,
        modelInfo: obj.model_info ? JSON.parse(obj.model_info) : null,
...
// POST /generate-persona - AI生成角色人格提示词
  ...
  "suggestedAvatarColor": "建议的头像颜色（hex格式，如#6b21a8）"
    ...
    try {
      ...
      return res.json({
        success: true,
        data: {
          identity: fullContent,
          sceneSetting: '',
          languageStyle: '',
          behaviorRules: '',
          suggestedSubtitle: subtitle || '',
          suggestedAvatarColor: '#6c5ce7',
          raw: fullContent,
        },
      });
    }
    ...
    res.json({
      ...
      data: {
        ...
        sceneSetting: personaData.sceneSetting || personaData.scene_setting || '',
...
router.post('/init-presets', async (_req: Request, res: Response) => {
  try {
    ...
    // Delete old preset characters that no longer match current preset names
    if (presets.length > 0) {
