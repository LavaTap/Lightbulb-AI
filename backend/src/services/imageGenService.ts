...
import { validateSize, resolvePixelSize, isRatioFormat } from '../utils/sizeValidator.js';
...
export async function generateImage(
  ...
  // 校验尺寸是否被当前服务商支持
  const sizeValidation = validateSize(size, config.provider);
  if (!sizeValidation.valid) {
    console.error('[Size Validation Error]', sizeValidation.message);
  ...
  // 解析比例→像素（DALL-E 需要具体像素尺寸）
  const pixelSize = resolvePixelSize(size, config.provider);
...
async function generateQwenImage(
  ...
  // Qwen API 不传 size 参数，通过 prompt 中的尺寸要求控制
...
async function generateTencentHunyuan(
  ...
  if (isLiteModel) {
    ...
    // 解析比例→腾讯混元分辨率格式（width:height）
    const pixelSize = resolvePixelSize(size, config.provider);
    const resolution = pixelSize.replace('x', ':');
...
async function generateTencentTokenHub(
  ...
  if (isLiteModel) {
    ...
    // 解析尺寸为 宽:高 格式
    const pixelSize = resolvePixelSize(size, config.provider);
    const hunyuanSize = pixelSize.replace('x', ':');
  ...
  // 解析尺寸为 宽:高 格式（混元接口要求）
  const pixelSize = resolvePixelSize(size, config.provider);
  submitBody.size = pixelSize.replace('x', ':');
...
async function generateGptImage2(
  ...
  // GPT-Image-2 直接接受比例（如 '1:1'、'9:16'、'16:9'）
  const aspectRatio = isRatioFormat(size) ? size : '1:1';
