...
description: >
  ...
  3. 通过 /guide update（增量）或 /guide update --full（全量）命令更新 DEVELOPER.md 开发者手册
...
| `/guide update` | **增量更新**：仅扫描 git 变更文件 → 读取变更内容 → 更新手册对应章节 → 输出更新报告 |
| `/guide update --full` | **全量更新**：全量扫描项目 → 重写完整 DEVELOPER.md → 输出更新报告 |
...
### 5.1 `/guide update` — 增量更新

当收到 `/guide update`（不带 `--full`）时，执行以下 **5 阶段流程**：

#### Phase 1 获取变更列表

```
→ 执行 git diff --name-only HEAD 获取已修改/新增/删除的文件列表
→ 过滤出需要关注的源文件（backend/src/**, frontend/src/**, 根目录配置文件）
→ 如果无变更文件，直接输出 "无代码变更，手册已是最新" 并结束
```

#### Phase 2 读取变更文件

```
→ 对每个变更文件执行 read_file，获取完整内容
→ 优先关注顺序：
...
```
→ 读取当前 DEVELOPER.md，定位变更文件对应的章节
→ 对比维度:
  • 新增/删除/重命名文件 → 更新 §3 目录结构
  • 函数签名/服务变更 → 更新 §4 后端架构
  • 前端组件/页面变更 → 更新 §5 前端架构
  • 模型/服务商变更 → 更新 §6 模型连接
  • 数据库字段增删 → 更新 §7 数据库表结构
  • API 接口/路由变化 → 更新 §8 API 接口文档
```

#### Phase 4 更新手册

```
→ 使用 replace_in_file 仅修改受影响的章节，保留其他章节不变
→ 版本号 +1，日期改为当天 YYYY-MM-DD
→ 在文档末尾 Changelog 追加本次变更记录
```
...
### 5.2 `/guide update --full` — 全量更新

当收到 `/guide update --full` 时，执行以下 **5 阶段流程**：
...
```
→ read_file + search_content 深入阅读所有源文件
→ 优先关注顺序：
  1. backend/src/services/*.ts   (AI 调用逻辑)
  2. backend/src/routes/*.ts      (API 接口)
  3. backend/src/database.ts      (数据模型)
...
```
→ write_to_file 重写完整 DEVELOPER.md
→ 版本号 +1，日期改为当天 YYYY-MM-DD
...
| 核心重构 | services/routes/ 小幅修改 | `🔔 核心模块已变，建议 \`/guide update\`` |
| 新增服务商 | 新 provider 类型或模型适配 | `🔔 兼容层已扩展，建议 \`/guide update\`` |
| DB 结构变动 | database.ts 建表语句变化 | `🔔 Schema 已变更，请务必 \`/guide update\`` |
| 大规模变更 | 多文件大量 diff（>5个文件） | `🔔 大规模变更，建议 \`/guide update --full\` 全量重写手册` |
...
开发者交互
    /guide               → 查询手册概览
    /guide <问题>         → 解答技术疑问
    /guide update         → 增量更新手册（仅扫描变更文件）
    /guide update --full  → 全量更新手册（重写整个文档）
```
